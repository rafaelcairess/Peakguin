...
func _ready() -> void:
	...
	if scene != null and CollectiblesManager.is_collected(scene.scene_file_path, str(get_path())):
		queue_free()
...
func _on_body_entered(body: Node2D) -> void:
	...
	var scene := get_tree().current_scene
	if scene != null:
		CollectiblesManager.mark_collected(scene.scene_file_path, str(get_path()))
